...
import { PANEL_NATURAL_WIDTH, PANEL_NATURAL_HEIGHT, DEVICE_PANEL_DIMENSIONS } from '@/lib/constants';
...
export default function TutorialRunner({
  ...
  // Resolve per-device panel dimensions
  const deviceDims = DEVICE_PANEL_DIMENSIONS[tutorial.deviceId];
  const panelWidth = deviceDims?.width ?? PANEL_NATURAL_WIDTH;
  const panelHeight = deviceDims?.height ?? PANEL_NATURAL_HEIGHT;
